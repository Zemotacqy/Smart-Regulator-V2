...
- Run ancestral section rollups (Stage C) *before* or *after* the reranking phase? The plan isolates generator-level rollups to occur *after* the top 5 nodes are selected by the reranker. This ensures the heavy cross-encoder scoring stage is kept completely isolated from the generator's large context blocks, saving significant CPU latency.

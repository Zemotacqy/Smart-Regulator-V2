...
async def ingest_document(pdf_path: str) -> UUID:
    """
    ...
    logger.info("ingest_document_started", file_name=file_name, path=pdf_path)
    ...
    # 4. Connect to database and start transaction to enforce idempotency
    async with get_db_connection() as conn:
        async with conn.transaction():
            ...
            # 7. Audit AST nodes
            ast_nodes = audit_ast_nodes(ast_nodes, total_raw_chars)
